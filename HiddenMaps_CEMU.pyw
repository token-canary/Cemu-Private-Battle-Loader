	...
	if region == 'USA':
		with open('0005000010176900.ini', 'r+') as file:
			...
			if '''\n[Entry]\ndescription=Stage Selector (PB)\naddress=0x1bd04ac8\ntype=int16\nvalue=''' in fileread:
				...
				if showmsgbox == True:
					tkinter.messagebox.showinfo(title='Not Found', message='''Address not found, perhaps you already removed it before.''')
					os.chdir(r'../')
	elif region == 'EUR':
...
verlabel = Button(master, text='v2.0.0', command=openaboutwindow,width=5)
verlabel.place(x=157,y=170)

alphacheckboxvar = tkinter.StringVar()
alphacheckbox = ttk.Checkbutton(master, text="Set First\n4 Players\n to Alpha", variable=alphacheckboxvar, onvalue='on',offvalue='off', state=DISABLED)
